...
public static class ModifyCardPlayResultLocationNewPatch
{
        ...
        ref IEnumerable<AbstractModel> modifiers)
    {
        ...
        var result = HookUtils.Modify<IModifyCardPlayResultLocation, CardLocationCompatibility>(
            combatState,
            new CardLocationCompatibility(player, pileType, position),
            (m, loc) => m.ModifyCardPlayResultLocationCompatibility(card, isAutoPlay, resources, loc),
...
public static class ModifyCardPlayResultLocationOldPatch
{
        ...
        ref IEnumerable<AbstractModel> modifiers)
    {
        var result = HookUtils.Modify<IModifyCardPlayResultLocation, CardLocationCompatibility>(
            combatState,
            new CardLocationCompatibility(card.Owner, __result.Item1, __result.Item2),
            (m, loc) => m.ModifyCardPlayResultLocationCompatibility(card, isAutoPlay, resources, loc),
            out var compatModifiers);
...
internal static class AfterModifyingLocationNewPatch
{
    ...
    public static Task Bridge(AbstractModel model, CardModel card, object boxedLocation)
    {
        ...
        var loc = new CardLocationCompatibility(
        ...
        static async Task Chain(Task orig, IModifyCardPlayResultLocation c, CardModel cd,
            CardLocationCompatibility l)
        {
            await orig;
            await c.AfterModifyingCardPlayResultLocationCompatibility(cd, l);
        }
    }
}
...
internal static class AfterModifyingLocationOldPatch
{
    ...
    public static Task Bridge(AbstractModel model, CardModel card, PileType pileType, CardPilePosition position)
    {
        ...
        return Chain(vanilla, compat, card,
            new CardLocationCompatibility(card.Owner, pileType, position));
...
{
    CardLocationCompatibility ModifyCardPlayResultLocationCompatibility(CardModel card, bool isAutoPlay,
        ResourceInfo resources,
    ...
    Task AfterModifyingCardPlayResultLocationCompatibility(CardModel card, CardLocationCompatibility cardLocation)
    {
